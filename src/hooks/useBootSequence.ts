...
interface UseBootSequenceParams {
  isSoundEnabled: boolean;
}

export default function useBootSequence({ isSoundEnabled }: UseBootSequenceParams) {
  ...
  useEffect(() => {
    const handleClick = (e: MouseEvent) => {
      ...
      if (e.target instanceof Element && (e.target.closest('.terminal-modal') || e.target.closest('.omni-palette'))) return;
  ...
  const startScan = useCallback((e?: SyntheticEvent) => {
