    ...
    <div className={`relative h-full flex flex-col rounded-xl overflow-hidden ${isFlipped ? 'pointer-events-none' : 'pointer-events-auto'}`} style={{ backfaceVisibility: 'hidden', WebkitBackfaceVisibility: 'hidden' }}>
      ...
      {!isFlipped && (
        <button
          type="button"
          tabIndex={tabIndex}
          className="card-focusable absolute inset-0 z-[5] w-full h-full rounded-xl cursor-pointer focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-accent-400 focus-visible:ring-offset-2 focus-visible:ring-offset-black bg-transparent border-0 p-0"
      ...
      <div className="h-full flex flex-col pointer-events-none" style={{ transformStyle: 'preserve-3d' }}>
        ...
        {/* Reachability + catalog meta */}
        <div
          className="absolute top-4 left-4 z-20 flex flex-col gap-1 transition-opacity duration-300"
          style={{ transform: 'translateZ(40px)' }}
        >
          <ProjectConnectivityBadge project={project} variant="card" />
          <div className={`flex flex-col gap-1 ${isHovered || project.featured ? 'opacity-100' : 'opacity-0'}`}>
            <ProjectMetaBadges project={project} variant="card" showConnectivity={false} />
          </div>
          {ping > 0 && (
        ...
        >
          ...
          <div className="flex items-center mb-3 relative z-10">
            ...
            <div className="flex-1">
              <h3 className="text-xl font-bold text-white tracking-wide group-hover:text-blue-300 transition-colors duration-300 flex items-center justify-between gap-2">
                <span className="flex items-center gap-2 min-w-0">
                  <DecryptText text={project.title} isHovered={isHovered} isVisible={isVisible} searchQuery={searchQuery} regex={regex} />
                  {project.featured && (
                    <span className="shrink-0 text-amber-400 text-sm" title="Featured">★</span>
                  )}
                </span>

                <ComplexityMeter variant="grid" score={complexityScore} />
              </h3>
              <p className="text-[10px] font-mono text-gray-500 mt-0.5">{project.year}</p>
            </div>
          </div>
