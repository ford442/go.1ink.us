...
export default function CardGridBack({ project, isFlipped, onClose }) {
  if (!isFlipped) {
    return (
      <div
        className={`${backShellClass} pointer-events-none`}
        style={backFaceStyle}
        aria-hidden="true"
      />
    );
  ...
  return (
    <div
      className={`${backShellClass} pointer-events-auto`}
      style={backFaceStyle}
      onClick={(e) => e.stopPropagation()}
    >
      <div className="flex items-center justify-between p-3 border-b border-accent-500/30 bg-accent-900/30">
        <div className="flex items-center gap-2">
          <span className="w-2 h-2 rounded-full bg-accent-400 animate-pulse shadow-[0_0_8px_rgba(var(--rgb-accent-400),0.8)]" />
      ...
      <div className="p-4 flex-1 overflow-y-auto scrollbar-hide" tabIndex={0}>
        ...
        <div className="mt-6 pt-4 border-t border-white/10 text-[10px] font-mono text-gray-400">
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden opacity-30 z-[6]" aria-hidden="true">
        <div className="scanline" style={{ animationDuration: '3s' }} />
      </div>
    </div>
  );
}
