import type { Category, CategoryButtonStyle } from './types';
...
export const CATEGORY_THEMES: Record<Category | 'default', string[]> = {
...
export const CATEGORY_SETS: Record<Category, Set<string>> = {} as Record<Category, Set<string>>;
(Object.entries(CATEGORIES) as [Category, string[]][]).forEach(([category, tags]) => {
  CATEGORY_SETS[category] = new Set(tags);
});

// Styles for active category and tag buttons
export const CATEGORY_BUTTON_STYLES: Record<Category | 'All' | 'Favorites' | 'default', CategoryButtonStyle> = {
  'Games': {
