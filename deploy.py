...
Deploy the Vite build to go.1ink.us via storage.noahcohn.com (Contabo VPS).
...
# Remote folder under the deploy base (empty = use PROJECT_NAME).
# Matches legacy SFTP target: go.1ink.us/
TARGET_FOLDER: str = os.getenv("TARGET_FOLDER", "")

# Set via environment: export DEPLOY_TOKEN="your_long_token_from_vps_env"
DEPLOY_TOKEN: Optional[str] = os.getenv("DEPLOY_TOKEN")

# Portal lives on go.1ink.us (not test.1ink.us).
DEPLOY_TARGET: str = os.getenv("DEPLOY_TARGET", "go")
